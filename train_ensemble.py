"""
Train an ensemble model combining multiple CNN backbones.

Usage:
    python train_ensemble.py --models resnet152 efficientnetb0
    python train_ensemble.py --models resnet152 efficientnetb0 vgg19 --epochs 100
...
def main():
    parser = argparse.ArgumentParser(description="Train an ensemble model for chart classification.")
    parser.add_argument("--models", type=str, nargs='+', required=True,
                        choices=list(AVAILABLE_MODELS.keys()),
                        help=f"Backbone models to ensemble. Options: {list(AVAILABLE_MODELS.keys())}")
    ...
    display_names = " + ".join([AVAILABLE_MODELS[m][1] for m in args.models])
    print(f"\nTraining Ensemble: {display_names}")
    ...
    # Build ensemble
    model = build_ensemble(args.models, NUM_CLASSES)
    ...
    # Callbacks
    ensemble_name = "+".join(args.models)
    weight_path = os.path.join(args.checkpoint_dir, f"ensemble_{ensemble_name}.weights.h5")
    callbacks = [
        EarlyStopping(
    ...
    # Evaluate
    print(f"\n{'='*50}")
    print(f"Ensemble Results: {display_names}")
